...
function check_update()
	...
	os.remove(path)
	local url = 'https://github.com/philpantach/PPantachh/blob/main/Update_Info.json'
	if isMonetLoader() then
		...
		if imgui.BeginTabBar('пон') then	
			if imgui.BeginTabItem(fa.HOUSE..u8' Главное меню') then
				...
				if imgui.BeginChild('##4', imgui.ImVec2(791 * settings.general.custom_dpi, 28 * settings.general.custom_dpi), true) then
					imgui.Columns(2)
					imgui.Text(fa.HAND_HOLDING_DOLLAR .. u8" Вы можете финансово поддержать автора скрипта (P.Pantach) донатом " .. fa.HAND_HOLDING_DOLLAR)
					imgui.SetColumnWidth(-1, 480 * settings.general.custom_dpi)
					imgui.NextColumn()
					...
					if imgui.BeginPopupModal(fa.SACK_DOLLAR .. u8' Поддержка разработчика', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						change_dpi()
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						imgui.CenterText(u8'Свяжитесь с Zaluper Pantacha:')
			...
			if imgui.BeginTabItem(fa.GEAR..u8' Настройки') then 
				...
				imgui.Separator()
				imgui.Text(fa.CIRCLE_USER..u8" Разработчик данного хелпера: MTG MODS(ReMake)")
				imgui.Separator()
				imgui.Text(fa.CIRCLE_INFO..u8" Установленная версия хелпера: " .. u8(thisScript().version))
				imgui.SameLine()
				imgui.Separator()
				imgui.Text(fa.CIRCLE_INFO..u8" Этот скрипт переделан на более доступную версию. По вопросам: remasterok(discord)")
				imgui.SameLine()
				if imgui.SmallButton(u8'Проверить обновления') then
					sampAddChatMessage('[Justice Helper] {ffffff}Проверка и установка обновлений недоступна в лаунчер версии!', message_color)
				end
				imgui.Separator()
				...
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. u8' Предупреждение ##delete', _, imgui.WindowFlags.NoResize ) then
					...
					imgui.Separator()
					if imgui.Button(fa.CIRCLE_XMARK .. u8' Нет, отменить', imgui.ImVec2(400 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						imgui.CloseCurrentPopup()
					end
					imgui.SameLine()
